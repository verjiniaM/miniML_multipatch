...
path = '/Users/verjim/miniML_multipatch/model_training/example_recordings_verji/'
...
for h in range(35, len(meta_df)):
    ...
    trace = MiniTrace.from_axon_file_MP(filepath=filename,
                        channel = chan,
                            scaling=scaling,
                            unit=unit,
                            first_point = 0, last_point = 5000) # cutting out the test pulse 
    ...
    t_rise = (baseline+(win_size * trace.sampling))/35
    t_decay = (baseline+(win_size * trace.sampling))/14
    ...
    for ind, location in enumerate(matching.indices):
        if location < trace.data.shape[0] - int(win_size*1.5):
            ...
            cell_label.append(filename + '_' + str(chan))
            idx.append(location)
            events.append(event)
            scores.append(1)
            event_counter += 1
...
#training_results = pd.DataFrame({'cell':cell_label ,'event_indx': idx,'event_score': scores})
...
save_dataset = '/Users/verjim/miniML_multipatch/model_training/output_training_verjinia/multiple_files_training_data_pt2.h5'
if save_dataset:
